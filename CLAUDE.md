...
**Done:** Delivery (phase 1) — `wholesale_shipments`/`wholesale_shipment_legs` (`backend/app/models/wholesale.py`), `/api/wholesale/shipments` (`backend/app/routers/wholesale_shipments.py`), business rules in `backend/app/services/wholesale/` (`colors.py`, `units.py`, `references.py`, `shipments.py` — the last porting `normaliseFlow` and friends from `frontend/.../wholesale/shipments.ts` line for line, since the server is now authoritative and the client copy is only instant feedback while typing). Seed data: `backend/scripts/seed_wholesale_demo.py` + `backend/scripts/data/wholesale_demo.json`, idempotent, run with `uv run --directory backend python scripts/seed_wholesale_demo.py --branch "Wholesale"`.
