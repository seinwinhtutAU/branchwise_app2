...
SQLAlchemy 2.0 style (`Mapped`/`mapped_column`) in `app/models/`, one file per entity, registered in `app/models/__init__.py` (required for Alembic autogenerate to see them). `Branch`/`User` are the base entities (`users.branch_id` nullable — not every role has a branch). `Product` is shared across `SaleLine`, `PurchaseLine`, and `StockLevel`. Money/quantity columns use `Numeric`, not `Float`. `alembic/env.py` reads the DB URL from `app.config.get_settings()` at migration time rather than from `alembic.ini`.
