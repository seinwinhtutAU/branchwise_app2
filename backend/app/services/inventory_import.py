...
def _parse_printed_at(cell: str) -> dt.datetime | None:
    ...
    try:
        return dt.datetime.strptime(rest, "%m/%d/%Y %I:%M:%S%p")
    except ValueError:
        return None
...
def parse_inventory_export_from_grid(rows: list[list[str]]) -> pd.DataFrame:
    ...
    for row_idx, row in enumerate(rows):
        ...
        if first.startswith("Printed"):
            printed_at = _parse_printed_at(first)
            continue
...
def parse_inventory_export(path: Path) -> pd.DataFrame:
    ...
    return parse_inventory_export_from_grid(rows)


def parse_inventory_upload(file_bytes: bytes, filename: str) -> tuple[list[list[str]], pd.DataFrame]:
    ...
    return rows, parse_inventory_export_from_grid(rows)
