...
Every confirm also writes the Import Detail preview to R2 as a small manifest, 50-row Cleaned/Original page objects, and separate warning-index objects under `imports/{batch.id}/preview/v1/`. Import Detail reads only the requested tab and page from those objects; its database `preview_data` remains as a compatibility fallback and for Import Health checks. To backfill existing batches, first run `python backend/scripts/migrate_preview_data_to_r2.py` to report the work, then rerun it with `--apply` to upload the missing preview objects. The script never removes the database fallback.
...
- **`GET /api/imports/history/{batch_id}`** — the same metadata as one row of the list plus one 50-row page for the requested `tab=clean|original`. It reads the corresponding R2 page when available and falls back to the batch's `preview_data` column for older imports. `GET /api/imports/history/{batch_id}/warnings` supplies warning positions separately, only when the UI needs them. Together they back the read-only Cleaned/Original Import Detail view without transferring both grids upfront. Same visibility rule and 404 behavior as the list/revert endpoints.
