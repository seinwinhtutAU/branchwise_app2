...
`POST /api/imports/general` (`app/retail/routers/imports.py`) stores any file exactly as uploaded (mirrored to R2 like the other imports) and creates an `ImportBatch` of type `general`. If the workbook is recognised, it also creates operational records in the same batch: daily cost summaries (`daily_cost_import.py`, needs the branch in the filename and date-named sheets), salary tables (`salary_import.py`) and zero-selling logs (`zero_selling_import.py`, columns date/time/shop/category/reason). Recognition is best-effort: an unrelated file is simply kept and creates no records. The response and the batch `summary` carry `salary_records_created`, `zero_selling_records_created` and `daily_cost_records_created`. The tables are described in [database-schema.md](../database-schema.md); they are read by `GET /api/daily-costs`, `GET /api/salaries` and `GET /api/zero-selling`, and shown as the **Salary**, **Zero selling** and **Daily cost** tabs of the Data Overview page.
