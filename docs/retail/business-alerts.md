...
| --- | --- | --- | --- |
| Daily import missing | Critical | After the business cutoff, today's Sale or Inventory file is missing. Purchase files are not required daily. Listed under **Data quality** (a missing file is a data problem, not a business decision), so it does not appear in the Summary's Recommended decisions. | Upload today's file with the **Upload Today's Sales/Inventory File** button in the alert's own panel. |
...
| Urgent reorder | Critical | An actively selling product has three or fewer days of stock cover. The panel lists the 10 most urgent, each with a recommended order of 30 days of selling minus what is on hand. | Reorder the listed products at the recommended quantity. The reorder itself happens outside the app, so this alert has no button. |
