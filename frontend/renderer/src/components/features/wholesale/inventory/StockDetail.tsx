...
import {
  Panel, ReadOnlyField, Reference, SectionLabel, StatusPill,
        ...
        {terms.map((term) => (
          <Fragment key={term.label}>
            ...
            <div className="flex-1 rounded-lg border border-border bg-bg-base px-4 py-3">
              ...
              <div className={cn("mt-1 font-bold tabular-nums", term.valueClass)}>
                {formatSets(term.pairs)}
              </div>
              <div className="mt-0.5 text-xs text-text-muted">{term.detail}</div>
            </div>
          </Fragment>
        ))}
      </div>
...
export function StockDetail({
  ...
  const [tab, setTab] = useState<"overview" | "orders" | "movement" | "pipeline">("overview");
  ...
  const pipelineStages = [
    { label: "At Supplier", detail: "Not yet shipped", pairs: record.at_supplier_pairs },
    { label: "In Transit", detail: "On the way", pairs: record.in_transit_pairs },
    { label: "At Receiving", detail: "Physically received at the gate", pairs: record.on_hand_pairs },
    { label: "Customer Allocated", detail: "Reserved against a customer order", pairs: record.allocated_pairs },
    { label: "Customer Owed", detail: "Ordered but not yet reserved", pairs: record.owed_to_customers_pairs },
    { label: "Delivered", detail: "Sent to customers", pairs: record.delivered_pairs },
    { label: "Lost", detail: "Written off from the pipeline", pairs: record.lost_pairs },
  ];
                ...
                <div className="grid gap-4 lg:grid-cols-2">
                  <div className="rounded-lg border border-border bg-bg-subtle/50 p-4">
                    ...
                    <dl className="grid gap-3 sm:grid-cols-2">
                      ...
                      <ReadOnlyField label="Group" value={GROUP_LABELS[line.product_group]} />
                      <ReadOnlyField
                  ...
                  {relatedOrders.length === 0 ? (
                    ...
                    <>
                      ...
                      <Tr className="bg-bg-subtle hover:bg-bg-subtle">
                        <Td colSpan={3} className="text-right font-semibold text-text-primary">
                          Total
                        </Td>
