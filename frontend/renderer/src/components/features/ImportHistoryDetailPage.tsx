...
function ImportHistoryDetailPage({ session, batchId, onBack }: Props): React.JSX.Element {
  ...
  useEffect(() => {
    ...
    fetch(`${apiBaseUrl}/api/imports/history/${batchId}`, {
      ...
      .catch(() => {
        if (!cancelled) {
          showToast('error', "Couldn't load this import — is the backend running?")
          onBack()
        }
      })
      .finally(() => {
    ...
    <div className="flex flex-col gap-4 animate-fade-in motion-reduce:animate-none">
      <div className="flex items-center gap-3">
        ...
        <div className="min-w-0">
          ...
          {detail && <p className="text-sm text-text-muted truncate">{detail.filename}</p>}
        </div>
      </div>
      ...
      {!loading && detail && (
        <>
          <div className="flex flex-wrap items-center gap-x-6 gap-y-2 text-sm text-text-muted">
            <span>
              Branch: <span className="text-text-primary">{detail.branch_name ?? '—'}</span>
            </span>
            <span>
              Uploaded by: <span className="text-text-primary">{detail.uploaded_by_name ?? '—'}</span>
            </span>
            <span>
          ...
          {Array.isArray(detail.summary.messages) ? (
            ...
            Object.entries(detail.summary).filter(([key]) => key !== 'batch_id').length > 0 && (
              <div className="flex flex-wrap gap-2">
                {Object.entries(detail.summary)
                  ...
                  .map(([key, value]) => (
                    ...
                    >
                      <span className="text-text-muted">{formatSummaryLabel(key)}: </span>
