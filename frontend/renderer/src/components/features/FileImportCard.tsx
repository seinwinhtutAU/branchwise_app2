...
function FileImportCard({ session, label, description, endpoint, icon, onFilesReady }: Props): React.JSX.Element {
  ...
  async function previewOne(file: File): Promise<PendingImport | null> {
    try {
      ...
      if (!response.ok) {
        ...
        showToast('error', `${file.name}: ${body?.detail ?? `Upload failed (${response.status})`}`)
        return null
      }

      const result: ImportPreviewResult = await response.json()
      return { id: crypto.randomUUID(), importLabel: label, endpoint, file, result }
    } catch {
      showToast('error', `${file.name}: Upload failed — is the backend running?`)
      return null
    }
  }
    ...
    <Card className="flex flex-col">
      ...
      {!uploading && (
        ...
        >
          ...
          <span className="text-xs">.csv, .xls, or .xlsx — pick several to review one after another</span>
        </button>
      )}

      {uploading && progress && progress.total > 1 && (
        <div className="flex flex-col gap-2 py-8 px-2" role="status" aria-label="Uploading">
          <ProgressBar
