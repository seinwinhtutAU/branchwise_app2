...
export function useStickyAbove(): {
  ...
  useEffect(() => {
    ...
    if (!el) return
    const observer = new ResizeObserver((entries) => setAboveHeight(entries[0].contentRect.height))
    observer.observe(el)
  ...
  return {
    aboveRef,
    containerStyle: { '--sticky-offset': `${aboveHeight}px` } as React.CSSProperties
  }
}
