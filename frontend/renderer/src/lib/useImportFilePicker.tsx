...
export function useImportFilePicker(
...
): { trigger: (context: PickContext) => void; input: React.JSX.Element; picking: boolean } {
  ...
  const input = (
    <input ref={fileInputRef} type="file" accept=".csv,.xls,.xlsx" className="hidden" onChange={handleChange} />
  )
